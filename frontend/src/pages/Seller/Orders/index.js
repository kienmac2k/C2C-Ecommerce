...
import ShopNav from "../Shops/ShopDetail/ShopDashboard";
...
const Dashboard = () => {
  ...
  const fetchOrderList = React.useCallback(async (shopId) => {
    try {
      const response = await vendorApi.getOrderOfShop(shopId);
  ...
  const handleApproveOrder = async (id) => {
    try {
      await vendorApi.updateOrder(id, { status: "accepted" });
  ...
  const handleCancelOrder = async (id) => {
    try {
      await vendorApi.updateOrder(id, { status: "cancelled" });
      fetchOrderList(shopId);
    } catch (e) {
      console.log("Error on calling api", e);
    }
