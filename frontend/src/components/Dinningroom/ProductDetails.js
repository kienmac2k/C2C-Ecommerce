...
const ProductDetails = () => {
  ...
  const dinning = productList.filter(
    (product) => product.category.replace(/\s+/g, "") === "dinning"
  );
  const theproduct = dinning.find((product) => product.id.toString() === id);

  const categories = dinning.filter(
